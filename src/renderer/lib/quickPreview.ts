...
export async function isNodeProject(taskPath?: string): Promise<boolean> {
  ...
  try {
    const res = await window.desktopAPI.fsRead(taskPath, 'package.json', 64 * 1024);
...
async function detectPackageManager(taskPath?: string): Promise<'bun' | 'npm'> {
  ...
  try {
    const bunLock = await window.desktopAPI.fsRead(taskPath, 'bun.lockb', 1);
    if (bunLock?.success) return 'bun';
  } catch {}
  try {
    const bunTextLock = await window.desktopAPI.fsRead(taskPath, 'bun.lock', 1);
...
export async function ensureCompose(taskPath?: string): Promise<boolean> {
  ...
  try {
    ...
    for (const file of candidates) {
      const res = await window.desktopAPI.fsRead(taskPath, file, 1);
  ...
  try {
    const res = await window.desktopAPI.fsWriteFile(
      taskPath,
      'docker-compose.yml',
      content,
      false
